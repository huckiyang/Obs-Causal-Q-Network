...
from .cevae_model import QNetwork, CEVAE_QNetwork, VAE_QNetwork, New_CEVAE_QNetwork
...
class DQNAgent():
    ...
    def __init__(self, state_size, action_size, seed, use_dueling=False, use_double=False, select_network=0):
        ...
        if select_network == 0:
            ...
            Net = New_CEVAE_QNetwork
        else:
            raise ValueError("Network type not supported.")
        self.qnetwork_local = Net(state_size, action_size, seed,
                                        use_dueling=use_dueling).to(device)
        self.qnetwork_target = Net(state_size, action_size, seed,
                                        use_dueling=use_dueling).to(device)
        print("Using {}.".format(self.qnetwork_local.name))
...
#         print("t_i: ", self.treatment_his)
    ...
    def act(self, state, eps=0.):
        ...
        with torch.no_grad():
            action_values = self.qnetwork_local(state)['py']
    ...
    def learn(self, experiences, gamma):
        ...
        rewards_ = rewards
#         print("rewards", rewards.to(device).shape)
...
#         print("treatments", treatments.to(device).shape)
        ...
        # self.soft_update(self.qnet, self.qnet_target, TAU)
                # Get max predicted Q values (for next states) from target model
        if self.use_double:
            action_values = self.qnetwork_local(next_states, treatments)['py']
            indices = torch.argmax(action_values.detach(),1)
            action_values = self.qnetwork_target(next_states, treatments)['py']
        ...
        model_out = self.qnetwork_local(states, treatments)
        action_values = model_out['py']
        Q_expected = action_values.gather(1, actions)
        
        # output = model(states) 
        # fake_state = Q_expected.
        ...
        # New CEVAE Loss
        if self.qnetwork_local.name == 'New_CEVAE_QNetwork':
            qy = model_out['qy'].gather(1, actions)
            z, qt, px, pt = model_out['z'], model_out['qt'], model_out['px'], model_out['pt']
            
            loss_px = F.mse_loss(px, states)
            loss_pt = nn.CrossEntropyLoss()(pt, treatments.long().squeeze())
            loss_z  = torch.pow(z, 2).mean()
            loss_qt = nn.CrossEntropyLoss()(qt, treatments.long().squeeze())
            loss_qy = F.mse_loss(qy, Q_targets)
            loss = loss + loss_px + loss_pt + loss_z + loss_qt + loss_qy

        # VAE Loss
        if self.qnetwork_local.name == 'VAE_QNetwork':
            z, px = model_out['z'], model_out['px']
            
            loss_px = F.mse_loss(px, states)
            loss_z  = torch.pow(z, 2).mean()
            loss = loss + loss_px + loss_z
